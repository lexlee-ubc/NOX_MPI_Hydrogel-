...
namespace HP_ALE
{
  ...
  //using PreconditionerType = TrilinosWrappers::PreconditionJacobi;
  //using AdDataType = TrilinosWrappers::PreconditionAMG::AdditionalData;
  //using NonlinearSolver = TrilinosWrappers::NOXSolver<VectorType>;
  enum class TestCase
  {
      case_1,
  };
  static const char *enum_str[] = {"case_1"};
  ...
  private:
   ...
   // SparsityPattern  volume_sparsity_pattern;
    ...
    VectorType old_solution;     // t=m-1
    VectorType current_solution; // newton iteration solution n, t=m

    VectorType dis_solution;         // newton, n+1  t=m
    VectorType dis_old_solution;     // t=m-1
    VectorType dis_current_solution; // newton iteration solution n, t=m

    //VectorType system_rhs;
    VectorType volume_solution;
    VectorType volume_old_solution;
    ...
    //needs to modify for the third phase
    struct PerTaskData
    {
   ...
   void
   assemble_local_hp_matrix(
      const typename DoFHandler<dim>::active_cell_iterator &cell,
      ScratchData &                                         scratch,
      PerTaskData &                                         copy_data);
    void
    copy_local_to_global_hp_matrix(const PerTaskData &copy_data);
    void
    ...
    solve_volume();
    void
    solve(const VectorType &rhs, VectorType &sol, const double tolerance);
  ...
  FluidStructureProblem<dim>::FluidStructureProblem(
    ...
    , viscosity(1)
    , vis_BM(1)
    , mu_s(83.333)///hydrogel lame
    , lambda_s(55.555)//hydrogel lame
    , beta_0(1)//
     ...
     if (cell_is_in_hydrogel_domain(cell_hp))
          {
              ...
              fe_values[extractor_displacement].get_function_gradients(current_solution,
                                                                       grad_u);

              // grad vs (mesh velocity)
              std::vector<Tensor<2, dim>> grad_vs(n_q_points);
              fe_values[extractor_mesh_velocity].get_function_gradients(current_solution,
                                                                        grad_vs);
...
//no need to modify
  ...
  template <int dim>
  void
  FluidStructureProblem<dim>::assemble_local_hp_matrix(
    ...
    PerTaskData &                                         copy_data)
  {
      ...
      for (unsigned int d = 0; d < dim; ++d)
      {
        ...
        if (cell_is_in_fluid_domain(cell_hp))
        {
            ...
            for (unsigned int f : GeometryInfo<dim>::face_indices())
                ...
                {
                    if (cell_is_in_fluid_domain(cell_hp->neighbor(f))) //interface 1
                    {
    ...
    template <int dim>
    void
    FluidStructureProblem<dim>::copy_local_to_global_hp_matrix(
            const PerTaskData &copy_data)
    {
    ...
    template <int dim>
    void
    FluidStructureProblem<dim>::compute_hp_jacobian_matrix(const VectorType &evaluation_point)
    {
        ...
        auto worker =
                [=](const typename DoFHandler<dim>::active_cell_iterator &cell,
                    ScratchData &                                         scratch,
                    PerTaskData &                                         copy_data) {
                    assemble_local_hp_matrix(cell, scratch, copy_data);
                };
        auto copier = [=](const PerTaskData &copy_data) {
             copy_local_to_global_hp_matrix(copy_data);
        };
    ...
    template <int dim>
    void
    FluidStructureProblem<dim>::assemble_local_hp_rhs(
            const typename DoFHandler<dim>::active_cell_iterator &cell_hp,
            ScratchData &                                         scratch,
            PerTaskData &                                         copy_data)
    {
        ...
        for (unsigned int d = 0; d < dim; ++d)
        {
            ...
            if (cell_is_in_fluid_domain(cell_hp))
            {
                ...
                for (unsigned int q = 0; q < n_q_points; ++q)
                {
                    ...
                    //pcout << "grad_vq norm" << linfty_norm(grad_vq) << std::endl;
                    // assemble local rhs
                    double tmp_rhs;
                    for (unsigned int i = 0; i < dofs_per_cell; ++i)
                    {
                                                    ...
                                                    VectorType &      residual)
    {
        ...
        auto worker =
                [=](const typename DoFHandler<dim>::active_cell_iterator &cell,
                    ScratchData &                                         scratch,
                    PerTaskData &                                         copy_data) {
                    assemble_local_hp_rhs(cell, scratch, copy_data);
                };
        auto copier = [=, &residual](const PerTaskData &copy_data) {
             copy_local_to_global_hp_rhs(copy_data, residual);
        };
    ...
    template <int dim>
    void
    FluidStructureProblem<dim>::solve(const VectorType &rhs, VectorType &sol,
                                      const double /*tolerance*/)
    {
        //SolverControl solver_control(rhs.size(), 1e-12);
        /*SolverControl solver_control(dof_handler.n_dofs(), 1e-12);
        TrilinosWrappers::SolverCG solver(solver_control);
        TrilinosWrappers::PreconditionAMG preconditioner;
        TrilinosWrappers::PreconditionAMG::AdditionalData data;
        preconditioner.initialize(jacobian_matrix, data);
        solver.solve(jacobian_matrix, sol, rhs, preconditioner);
        constraints_newton_update.distribute(sol);*/

        SolverControl                    solver_control;
        TrilinosWrappers::SolverDirect::AdditionalData data(false,
                                                            "Amesos_Mumps");
        TrilinosWrappers::SolverDirect DirectSolver(solver_control, data);
        DirectSolver.solve(jacobian_matrix, sol, rhs);
        //constraints_newton_update.distribute(sol);
        //jacobian_matrix.vmult(sol, rhs);
        //constraints_hp.distribute(sol);
    }
    ...
    FluidStructureProblem<dim>::nonlinear_solve()
    {
        ...
        const double target_tolerance = 1e-8;
        ...
        /*Teuchos::ParameterList& stepParams = searchParams.sublist("Backtrack");
        stepParams.set("Default Step", 1e-3);*/
        ...
        TrilinosWrappers::NOXSolver<VectorType> solver(additional_data, nlParamsPtr);
        ...
        solver.setup_jacobian = [&](const auto &evaluation_point) {
            compute_hp_jacobian_matrix(evaluation_point);
        };

        solver.solve_with_jacobian = [&](const auto &rhs,
                                          auto &dst,
                                          const double tolerance) {
            solve(rhs, dst, tolerance);
        };

        solver.solve(dis_current_solution);
        //constraints_hp.distribute(dis_current_solution);
        //current_solution = dis_current_solution;
    }
